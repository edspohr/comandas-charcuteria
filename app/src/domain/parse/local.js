    ...
    for (const { re, unit } of QTY_PATTERNS) {
        ...
        if (m) {
            ...
            if (Number.isFinite(n) && n > 0)
                return { qty: n, unit, matchedText: m[0] };
        }
    }
    return null;
}
function pickFormat(product, raw, extractedUnit) {
    ...
    if (extractedUnit) {
        ...
        if (bySearchUnit.length === 1)
            return bySearchUnit[0];
...
function extractNotes(raw, qtyMatch) {
...
export function parseLocal(text, products) {
    const haystack = [];
    ...
    for (const raw of splitBlocks(text)) {
        ...
        const status = scoreOk && qtyOk && !otherStrong ? 'verified' : 'review';
